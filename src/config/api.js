...
export const OffiAccount = data => { return http.post(`${wechatServiceApi}/offi-account/user/${data.code}?corpId=${data.corpId}&userNo=${data.userNo}`) } // 添加公众号授权后获取的用户信息
...
export const OpportunitiesStageList = data => { return http.get(`${customerServiceApi}/business-opportunities/stages/${data.corpId}?isEnd=${data.isEnd}`) } // 获取商机阶段概要信息
...
export const UserMessageReceive = data => { return http.get(`${customerServiceApi}/clueCustomerFollowUser/user-message-receive?clueCustomerNo=${data}`) } // 是否有新信息
    // 全部动态
export const ArticleLists = data => { return http.post(`${customerServiceApi}/my/queryFollowMsgPage`, data) }
export const SaleDocumentLists = data => { return http.post(`${customerServiceApi}/my/queryFollowMsgPage`, data) }
export const PosterLists = data => { return http.post(`${customerServiceApi}/my/queryFollowMsgPage`, data) }