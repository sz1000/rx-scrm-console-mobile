<template>
  <div class="clueWarp">
    ...
    <van-action-sheet v-model="show" :lock-scroll='false' @cancel='cancelIcon' @click-overlay='cancelIcon' title="删除协助人">
        ...
        <div class="lrBox">
          <div class="leftBox">
            <el-checkbox-group v-model="checkList" @change="changeRightList" :key="index" class="checkBoxlabelEdit" v-for="(item, index) in yiList">
              <el-checkbox @change="checkedChange" class="checkClass" :label="item.id">
                <span>{{item.name}}</span>
              </el-checkbox>
            </el-checkbox-group>
          </div>
    ...
    <van-action-sheet v-model="showB" :lock-scroll='false' @cancel='cancelIconAdd' @click-overlay='cancelIconAdd' title="添加协助人">
      <div class="vanContent">
        ...
        <van-action-sheet v-model="bumenPop" title="选择部门">
          <van-picker title="" show-toolbar :columns="bumenList" @confirm="onConfirm" @cancel="onCancel" @change="onChange" value-key="name" />
        </van-action-sheet>
        ...
        <div class="boxTop" style="margin-top: 30px;">
          ...
          <van-action-sheet v-model="yuangongPop" title="选择员工">
            <van-picker title="" show-toolbar :columns="heperList" @confirm="onConfirmYuangong" @cancel="onCancelYuangong" @change="onChangeYuangong"
                        value-key="name" />
          </van-action-sheet>
        ...
        <div class="btnBox">
          <div class="buttonWarp">
            <span class="cancel" @click="closeDialogAdd">取消</span>
...
export default {
  ...
  methods: {
    ...
    saveDialogAdd() {
      ...
      this.$network
        .post('/customer-service/clueCustomerFollowUser/addFollowUser', params)
        .then((res) => {
          console.log(res)
          if (res.code == 'success') {
            this.showB = false
            let name = this.oneList[0].name,
              nameArr = []
    ...
    getBumenSec() {
      ...
      _this.$network
        .get('/user-service/userdepartment/getAlllist')
        .then((res) => {
