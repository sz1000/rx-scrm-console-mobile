<template>
  <div class="detail_wrap">
        ...
        <div class="val">
          <div class="input_box">
            <input
              ...
              maxlength="12"
              type="text"
              @input="detail.name = detail.name.replace(' ', '')"
              placeholder="请输入活码名称"
            />
        ...
        <div class="val">
          <div class="input_box">
            <input
              ...
              maxlength="12"
              type="text"
              @input="detail.groupName = detail.groupName.replace(' ', '')"
              placeholder="请输入群名称"
            />
      ...
      <div class="item" v-if="!detail.switchStatus">
        ...
        <div class="val">
          <div class="input_box">
            <input
              class="input"
              v-model="detail.addHint"
              maxlength="12"
              type="text"
              @input="detail.addHint = detail.addHint.replace(' ', '')"
              placeholder="请输入入群提示"
            />
      ...
      <div class="item" v-for="(item, index) in addCodeData" :key="index">
        ...
        <div class="val">
          <div class="upload_wrap">
            ...
            <div class="flex_center">
              <div class="upload_box">
                ...
                <input
                  type="file"
                  class="file"
                  @change="uploadFun($event, '2', 'png,jpg,jpeg', index)"
                  accept=".png,.jpg,.jpeg"
                />
              </div>
              <div
                class="select_box"
                v-if="detail.switchStatus"
                @click="openPicker('group', index)"
              >
                <input
                  class="input"
            ...
            <div class="text_box">
              群上限人数<input
                v-model="item.groupTotal"
                @change="maxChangeFun(item.groupTotal, index)"
                @input="item.groupTotal = item.groupTotal.replace(/[^\d]/g, '')"
                maxlength="3"
                placeholder=""
              />人
            </div>
    ...
    <!-- 单选弹窗 -->
    <van-popup v-model="showPicker" round position="bottom" class="van_popup">
      <van-picker
        show-toolbar
        :columns="pickerList"
        value-key="name"
...
import {
  ...
  livecodegroup_upload,
  livecodegroup_add,
  livecodegroup_update,
  livecodegroup_getByNo,
} from "@/api/group";
export default {
  components: {
    TopTitle,
