...
<template>
  <div class="microCode group-setting-warp" v-loading="loading">
        ...
        <div class="form">
          <el-form ref="form" :rules="rules" :model="baseForm">
            ...
            <el-form-item label="任务名称:" prop="taskName">
              <el-input v-model="baseForm.taskName" placeholder="请输入任务名称" show-word-limit maxlength="30" :onkeyup="
                ...
                <el-form-item label="添加时间:" class="tamedata" prop="cusAddTime">
                  <div style="display: flex">
                    <div @click="selectCusDateTime('Begin')">
                      <el-input placeholder="请选择日期" suffix-icon="el-icon-date" readonly v-model="baseForm.cusAddBeginTime">
                      </el-input>
                    </div>
      ...
      <div class="group-content miassContent">
        ...
        <el-form :model="FormData" :rules="rulesCenter" ref="form1" class="centerListTime">
          <el-form-item label="入群引导语:" prop="sendMsg">
            <el-input class="marB-24" type="textarea" placeholder="请输入入群引导语" v-model="FormData.sendMsg" maxlength="300" show-word-limit>
            </el-input>
          </el-form-item>
        </el-form>
        <!-- 附件 -->
        <div class="appendix-list">
          <div v-for="(item, index) in appendixList" :key="index" style="display: flex">
            ...
            <div class="item marB-24 itemLablewarp">
              ...
              <template>
                <van-uploader v-model="item.picList" :max-count="1" :max-size="2 * 1024 * 1024" @oversize="onOversize" :after-read="
                ...
                <div class="selsctWarpList">
                  <el-select v-model="item.labelTime" placeholder="请选择" @change="labelGroupList(item)">
                    <el-option v-for="item in labelListTimeData" :key="item.id" :label="item.name" :value="item.chatId">
                    </el-option>
                  </el-select>
                </div>
              </template>
    ...
    <van-popup v-model="chooseDateTime" position="bottom">
      <van-datetime-picker @cancel="chooseDateTime = false" v-model="sendDateTime" :type="activeChoose" @confirm="confirmDateTime"
                           :title="activeChoose === 'date' ? '选择年月日' : '选择时间'" />
    </van-popup>

    <!-- 用户添加起止时间 -->
    <van-popup v-model="chooseCusAddDate" position="bottom">
      <van-datetime-picker :max-date="maxDate" @cancel="chooseCusAddDate = false" v-model="cusAddTime" type="date" @confirm="confirmCusAddDate"
                           title="选择年月日" />
    </van-popup>
    <!-- 选择客户标签 -->
