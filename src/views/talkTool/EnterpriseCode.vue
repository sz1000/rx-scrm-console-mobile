<template>
  <div class="microCode">
    ...
    <select-dialog
      v-model="dialog"
      :data="columns"
      :keys="select.key" :columnValue="select.value" 
      @confirm="selectedFun"
    ></select-dialog>
  </div>
</template> 
...
import { SelectDialog } from "../customer/components";
...
export default {
  ...
  methods: {
    ...
    selectedVal(val) {
      this.dialog = true;
      this.columns = this.channelList;
    },
    selectedFun(val) {
        let arr = val;
        for (let i = 0; i < arr.length; i++) {
          this.addForm.chId=arr[i].chId
          this.addForm.channel=arr[i].name
        }
    },
    getUserAuth() {
